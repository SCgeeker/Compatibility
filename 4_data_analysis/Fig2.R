...
O_DF <- data.frame(EXP = rep(c("A","B"),c(8,6)), rbind(EXPA_Table, EXPB_Table))

O_DF$SOA = c(rep(c(0,500,1000,1500),each=2), rep(c(0,250,500),each=2))
O_DF$RT.se = O_DF$RT.sd/sqrt(O_DF$n)
O_DF$PE = (1 - O_DF$ACC)*100
limits <-  aes(ymax = RT + RT.se, ymin = RT - RT.se)

Fig2_RT <- ggplot(O_DF, aes(SOA, RT, group = paste(EXP, Compatibility)) ) + geom_line(aes(colour = EXP, linetype = Compatibility) ) + geom_errorbar(limits) + geom_point(aes(colour = EXP, shape = EXP, size = 1.5)) + scale_y_continuous(limits=c(600,1000)) + labs(x="SOA",y="Reaction Time(ms)",title = "Recover Hommel's Figure 2") + guides(size = FALSE, shape = FALSE) + theme_bw() + theme(legend.justification = c(1, 1), legend.position = c(1, 1))

Fig2_PE <- ggplot(O_DF, aes(SOA, PE, group = paste(EXP, Compatibility)) ) + geom_line(aes(colour = EXP, linetype = Compatibility) ) + geom_point(aes(colour = EXP, shape = EXP, size = 1.5)) + scale_y_continuous(limits=c(0,100))+ labs(y="Error(%)") + theme_bw() %+replace% theme(panel.background = element_rect(fill = NA))
